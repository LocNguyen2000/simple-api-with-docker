        ...
        include: [
          {
            model: OrderDetail,
            required: true,
            attributes: ['quantityOrdered', 'priceEach', [sequelize.literal('(quantityOrdered * priceEach)'), 'TotalDetailPayment']],
          },
        ],
        transaction: t,
      });
...
export const updateOrder = async (req, res, next) => {
  ...
  try {
    ...
    if ((orderInstance.status != ORDER_STATUS.IN_PROCESS || (status != ORDER_STATUS.DISPUTED && status != ORDER_STATUS.SHIPPED && status != ORDER_STATUS.ON_HOLD)) && (orderInstance.status != ORDER_STATUS.ON_HOLD || status != ORDER_STATUS.SHIPPED) && (orderInstance.status != ORDER_STATUS.DISPUTED || status != ORDER_STATUS.RESOLVED) && (orderInstance.status != ORDER_STATUS.RESOLVED || status != ORDER_STATUS.SHIPPED)) {
