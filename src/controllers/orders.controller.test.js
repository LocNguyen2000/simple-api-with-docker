...
import { mockOrderQuery, mockOrderRequest } from '../tests/mocks/orderData.mjs';
import { ORDER_STATUS, ROLE } from '../config/variables.mjs';
import { mockProduct } from '../tests/mocks/productData.mjs';
...
describe('Order controller', () => {
  describe('get', () => {
    ...
    test('success: get own customers as staff', async () => {
      ...
      Customer.findAll.mockResolvedValue([{ customerNumber: mockCustomer.customerNumber }, { customerNumber: mockCustomer.customerNumber + 1 }]);
      Order.findAndCountAll.mockResolvedValue([mockOrderQuery]);
      ...
      expect(Order.findAndCountAll).toHaveBeenCalledWith(expect.objectContaining({ where: expectedQuery }));
      expect(result.status.mock.calls[0][0]).toEqual(200);
      expect(result.json.mock.calls[0][0]).toEqual({ data: [mockOrderQuery] });
    });
    ...
    test('error: check others staff customer as staff', async () => {
      ...
      Customer.findAll.mockResolvedValue([{ customerNumber: mockCustomer.customerNumber + 1 }, { customerNumber: mockCustomer.customerNumber + 2 }]);
